...
trend.nations <- function(class.sample = "Ground_vehicles", y = "rb_win_rate", rb_br.sample = "9.7 ~ 10.7") {
  # read the data generated by function cache_br_flat_df in integration/concator.py
  data <- read.csv(cache.1.file.path) %>% as.data.table
...
trend.nations.battles <- function(class.sample = "Ground_vehicles", y = "rb_battles_sum", rb_br.sample = "1.0 ~ 10.7") {
  # read the data generated by function cache_br_flat_df in integration/concator.py
  data <- read.csv(cache.all.file.path) %>% as.data.table
  ...
  data.sample %>% ggplot +
    ...
    ggtitle(paste("Battles Trend for", class.sample, rb_br.sample, sep = " "))
}
